...
class DscResourceStore
  ...
  def self.get_resource(resource_name)
    ...
    current_resource = add_resource(resource_name) if current_resource.nil?
  ...
  def self.find_resource(resource_name)
    if @@resource_data && @@resource_data.key?(resource_name.downcase)
  ...
  def self.add_resource(resource_name)
    if find_resource(resource_name)
      fail 'Resource #{resource_name} already exists'
    end

    @@resource_data = {} if @@resource_data.nil?
